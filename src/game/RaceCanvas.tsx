...
function RaceCanvas({
  ...
  useEffect(() => {
    ...
    for (const rival of rivalsRef.current) carSprite(rival.car)
    ...
    const posesDosFantasmas = new Map<string, { pose: CarPose; palette: CarPalette; lateral: number }>()
     ...
     */
    const drawGhost = (rival: RaceRival, distanceAhead: number, lateral: number, faded: boolean, dt: number) => {
      ...
      if (!estado) {
        estado = {
          pose: { tilt: 0, squash: 0, steer: 0, boost: 0, jitter: 0 },
      ...
      estado.pose.steer += (volante - estado.pose.steer) * (1 - Math.exp(-Math.max(0, dt) / 0.18))
      estado.pose.tilt = estado.pose.steer * 0.075 * forcaDoMovimento

      estado.palette.alpha = GHOST_PALETTE.alpha * (faded ? 0.5 : 1)
      // A pintura do rival vem tingida de azul: mesmo com os dois no mesmo
      // carro, o fantasma nunca se confunde com o próprio.
      const pintura = carSprite(rival.car).fantasma
      drawCar(ctx, x, projected.y, scale, estado.palette, estado.pose, pintura)
    }
    ...
    const draw = (frame: number) => {
      ...
      // da simulação porque a melhor esteira disponível entra na física.
      ...
      for (let indice = pedras.length - 1; indice >= 0; indice -= 1) {
        ...
        while (fantasmasVisiveis[proximoFantasma]?.ahead > ahead) {
          ...
          drawGhost(fantasma.rival, fantasma.ahead, fantasma.sample.lateral, fantasma.sample.stale, dt)
          proximoFantasma += 1
        }
